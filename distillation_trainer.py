...
class DistTrainer(Trainer):
    ...
    def get_batch_loss_metrics(
        ...
        metrics = {}
        policy_logits, loss_mask = self.concatenated_forward(model, batch)

        with torch.no_grad():
            if self.ref_model is None:
                with self.null_ref_context():
                    ref_logits, _ = self.concatenated_forward(self.model, batch)
            else:
                ref_logits, _ = self.concatenated_forward(self.ref_model, batch)

        def cross_entropy_loss(logits, ref_logits, loss_mask):
            # Flatten the tokens
            loss_fct = nn.CrossEntropyLoss(reduction="none")
            target = ref_logits.softmax(dim=-1)
            loss = loss_fct(logits.view(-1, logits.shape[-1]), target.view(-1, target.shape[-1]))
            loss_mask = loss_mask.to(logits.device)
            return loss * loss_mask.view(-1)

        losses = cross_entropy_loss(policy_logits, ref_logits, loss_mask)
